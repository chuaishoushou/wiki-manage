...
> 跨平台:本工作流(正文)三家通用;CC 侧由 plugin 提供 MCP 工具,Codex/Cursor 连同一 MCP。MCP 工具完整名形如 `mcp__wiki__wiki_*`。
>
...
调 `wiki_suggest_path`(传摘要),拿到 `domain / page_type / slug / confidence`。这是确定性算法,任何机器同结果——把它当建议,最终由第 4 步校验闸拍板。
...
- 已存在相关页 → **优先重写整合**,不要新建重复(先 `wiki_search` 找相似)。
...
落盘前调 `wiki_validate`(CLI: `wiki-cli validate <path>`)。**有 error 不予落盘**,先补齐。
- tags 必须取自白名单,命中同义词先归并;禁止用 tag 表达状态。

## 第 5 步:落盘 + 连通 + 审计

1. `Write` 目标文件(用 file tools,不经 MCP)。
