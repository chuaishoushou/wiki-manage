...
个人库 = `{{WIKI_ROOT}}`,**知识写入层 = `{{WIKI_CONTENT}}`**(v2 嵌套库是 `<库根>/wiki`,v3 扁平库即库根;写错层 CLI 会扫不到);`wiki-cli` = `{{WIKI_CLI}}`。
...
3. **写盘**:直接 Write,或 `wiki-cli new <type> <slug> --domain <主题>` 生成骨架再填正文。
   - 文件名小写 kebab-case;frontmatter 按库协议(`{{WIKI_ROOT}}/AGENTS.md`;有 `_vocabulary.md` 的库以其必填集为准)。
