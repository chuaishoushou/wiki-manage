...
**两类入口**:`wiki-query` / `wiki-ingest` / `wiki-lint` 是 **skill**(直接说"查 wiki…/把这条记进知识库/给知识库体检"即可触发);`/wiki-learn` / `/wiki-help` 是**斜杠命令**(Codex/Cursor 无斜杠命令,说同样的话或跑 `wiki-cli guide <op>` 等价)。底层都是 wiki-cli + 直接读写库文件。
...
| 体检 | "给知识库体检"(skill wiki-lint);环境层 `wiki-cli doctor`,内容层 `wiki-cli lint` |
