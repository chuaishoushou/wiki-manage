...
- **记知识**:写入 `{{WIKI_ROOT}}/domains/<主题>/`(新主题直接建目录,不受限制;拿不准放 `inbox/`),并在 `log.md` 追加一行。
- **学团队知识**:斜杠命令 `/wiki-learn`;无斜杠命令的工具用 CLI 等价流程——`wiki-cli learn --pull` 看增量 → 逐页读原文分类写入个人库 `domains/`(页内带 `learned_from`/`learned_commit` 溯源)→ `wiki-cli learn --mark <commit>` 记水位。
