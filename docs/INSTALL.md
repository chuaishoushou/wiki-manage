...
| **Codex** | 无插件机制,`wiki-init` 写规则指针(`~/.codex/AGENTS.md`)+ 设库路径;skill 自动触发不保证,一致性靠规则指针 + wiki-cli |
| **Cursor** | 每个项目写一次 `.cursor/rules/wiki.mdc` 规则指针;靠 `@wiki` 或让 AI 直接 Read/Grep + 调 wiki-cli |

**跨平台一致的内核** = 规则指针 + wiki-cli,三家结果一致:AI 直接读本地 .md 查知识,用确定性的 CLI 做协议/检索/校验。
